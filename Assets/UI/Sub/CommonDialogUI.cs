...
namespace UI.Sub {
    public class CommonDialogUI : UIBase {
        ...
        public override void Init() {
            base.Init();
            // 初始隐藏所有动态显示的元素
            if (blackGround) blackGround.SetActive(false);
            if (whiteGround) whiteGround.SetActive(false);
            if (mode_One) mode_One.SetActive(false);
            if (mode_Two) mode_Two.SetActive(false);
            // if (mode_One_back) mode_One_back.SetActive(false);
            // if (mode_Two_enter) mode_Two_enter.SetActive(false);
            // if (mode_Two_back) mode_Two_back.SetActive(false);
        }

        public override void Show() {
            base.Show();
            Debug.Log($"commDialog Show");
        ...
        private void Initialization(GroundType type, Mode mode, string message) {
            ...
            Debug.Log($"commDialog Initialization, type={this.type}");

            GetGround().SetActive(true);
            GetMode().SetActive(true);
            Debug.Log($"commDialog Initialization2, Ground={GetGround()}, mode={GetMode()}");
